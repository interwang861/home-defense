...
const DIFFS: [number, string][] = [[0.7, '轻松'], [1, '普通'], [1.4, '困难']];

export function BestiaryPanel({ s, act }: { s: GameState; act: Act }) {
  const n = s.attackCount + 1;
  const d = s.difficulty ?? 1;
  const mul = monsterMultipliers(n, s.buildings.base, d);
  const minT = monsterMinTier(n);
  const pow = defensePower(s);
  const rate = waveBudgetRate(s, false);
  return (
    <div className="space-y-1.5 p-3">
      <div className="space-y-1 rounded-lg border border-rose-500/30 bg-rose-950/20 p-2">
        <div className="text-xs font-bold text-rose-200">第 {n} 次进攻强度</div>
        <Stat label="生命倍率" now={`×${mul.hp.toFixed(2)}`} />
        <Stat label="攻击倍率" now={`×${mul.atk.toFixed(2)}`} />
        <Stat label="怪物数量系数" now={`${rate.toFixed(2)}/秒`} />
        <Stat label="7天周期加成" now={`第 ${mul.cycle + 1} 周期 ×${mul.cycleMul.toFixed(2)}${mul.cycle >= 20 ? '（已封顶 +20%）' : ''}`} />
        <Stat label="本周期内递增" now={`×${mul.stepMul.toFixed(3)}`} />
        <Stat label="我方防守战力" now={`${fmt(pow.dps)} 秒伤 / ${fmt(pow.hp)} 总血`} />
        <p className="pt-1 text-[10px] leading-relaxed text-slate-400">
          怪物个体偏弱、数量很多：主基地每级生命 +2.5%、攻击 +1.5%，每 7 天再 +1%（最多 +20%）。
          兵营人口按等级节点增长，战车上限是兵营的 115%，空军是 121%。
          {minT > 0 && <> 目前 <b className="text-rose-300">{MONSTERS[minT].name}</b> 以下的杂兵已经不再出现。</>}
        </p>
        <div className="flex items-center gap-1 pt-1">
          ...
          {DIFFS.map(([v, label]) => (
            <button
              ...
              className={cn('rounded px-2 py-0.5 text-[11px] font-bold', d === v ? 'bg-rose-600 text-white' : 'bg-slate-800 text-slate-300')}
            >
              {label}
            </button>
          ))}
