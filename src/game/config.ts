...
/** 兵营人口锚点。中间等级按相邻锚点线性取整，保证锚点等级精确命中 */
const BARRACKS_POP: [number, number][] = [
  [1, 20], [2, 35], [5, 55], [10, 130], [20, 300], [30, 500], [40, 750], [50, 900],
];

export function barracksPop(level: number): number {
  if (level <= 0) return 0;
  const last = BARRACKS_POP[BARRACKS_POP.length - 1];
  if (level >= last[0]) return last[1];
  let i = 0;
  while (BARRACKS_POP[i + 1][0] < level) i++;
  if (BARRACKS_POP[i][0] === level) return BARRACKS_POP[i][1];
  const [l0, p0] = BARRACKS_POP[i];
  const [l1, p1] = BARRACKS_POP[i + 1];
  return p0 + Math.round(((p1 - p0) * (level - l0)) / (l1 - l0));
}

export function popCap(cat: UnitCat, lvl: number): number {
  const base = barracksPop(lvl);
  if (cat === 'veh') return Math.round(base * 1.15);
  if (cat === 'air') return Math.round(base * 1.21);
  return base;
}
...
export const MONSTERS: MonsterDef[] = [
  { id: 'goblin', name: '哥布林', hp: 24, atk: 3, speed: 42, range: 20, cd: 1, splash: 0, hitsAir: false, cost: 1.1, size: 32, desc: '成群结队的小怪，弱小但数量多', shot: '#84cc16' },
  { id: 'dog', name: '野狗', hp: 20, atk: 4, speed: 85, range: 18, cd: 0.8, splash: 0, hitsAir: false, cost: 1.1, size: 34, desc: '速度很快，会迅速冲到基地', shot: '#a16207' },
  { id: 'wolf', name: '野狼', hp: 48, atk: 7, speed: 75, range: 20, cd: 0.9, splash: 0, hitsAir: false, cost: 2.2, size: 40, desc: '凶猛迅捷的掠食者', shot: '#9ca3af' },
  { id: 'bison', name: '野牛', hp: 180, atk: 13, speed: 38, range: 24, cd: 1.4, splash: 20, hitsAir: false, cost: 5.5, size: 52, desc: '皮糙肉厚，冲撞造成范围伤害', shot: '#78350f' },
  { id: 'boar', name: '野猪', hp: 140, atk: 16, speed: 62, range: 22, cd: 1.1, splash: 0, hitsAir: false, cost: 5.5, size: 46, desc: '獠牙锋利，冲锋迅猛', shot: '#57534e' },
  { id: 'gorilla', name: '暴怒猩猩', hp: 400, atk: 26, speed: 48, range: 130, cd: 1.6, splash: 25, hitsAir: true, cost: 14, size: 56, desc: '投掷巨石，可攻击空中单位', shot: '#a8a29e' },
  { id: 'zombie', name: '丧尸', hp: 300, atk: 18, speed: 36, range: 20, cd: 1, splash: 0, hitsAir: false, cost: 9, size: 42, desc: '不知疼痛，缓慢而坚韧', shot: '#65a30d' },
  { id: 'jiangshi', name: '僵尸', hp: 560, atk: 32, speed: 52, range: 22, cd: 1.2, splash: 0, hitsAir: false, cost: 18, size: 44, desc: '清朝僵尸，蹦跳前进，力大无穷', shot: '#facc15' },
  { id: 'ultraman', name: '假奥特曼', hp: 2600, atk: 90, speed: 40, range: 175, cd: 2, splash: 55, hitsAir: true, cost: 80, size: 70, desc: '山寨光之巨人，发射范围光线', shot: '#fde047', beam: true },
  { id: 'spider', name: '蜘蛛机器人', hp: 1000, atk: 18, speed: 56, range: 165, cd: 0.35, splash: 0, hitsAir: true, cost: 48, size: 58, desc: '机械蜘蛛，高速激光可打击空中', shot: '#f43f5e', beam: true },
  { id: 'terminator', name: '终结者', hp: 3000, atk: 55, speed: 38, range: 225, cd: 0.45, splash: 0, hitsAir: true, cost: 120, size: 56, desc: '来自未来的杀戮机器', shot: '#ef4444' },
];
...
/**
 * 怪物个体偏弱，靠数量形成大规模战斗。
 * 主基地每级生命 +2.5%、攻击 +1.5%；每 7 天 +1%，最多 +20%。
 */
export function monsterMultipliers(n: number, baseLevel: number, difficulty = 1) {
  const L = Math.max(1, baseLevel);
  const cycle = Math.floor((n - 1) / BIG_EVERY);
  const step = (n - 1) % BIG_EVERY;
  const lvlHp = 1 + 0.025 * (L - 1);
  const lvlAtk = 1 + 0.015 * (L - 1);
  const cycleMul = 1 + 0.01 * Math.min(cycle, 20);
  const stepMul = 1 + 0.001 * step;
  const d = difficulty;
  return {
    hp: lvlHp * cycleMul * stepMul * d,
    atk: lvlAtk * cycleMul * stepMul * d,
    cycle,
    cycleMul,
    stepMul,
    lvlHp,
...
/** 杂兵退场放慢，方便一直用大量弱怪铺场 */
export function monsterMinTier(n: number): number {
  return Math.max(0, Math.min(4, Math.floor((n - 80) / 40)));
}

/** 评估玩家当前的防守实力（部队 + 炮塔 + 主基地主炮） */
export function defensePower(gs: GameState) {
  let dps = 0;
  let hp = 0;
  for (const [id, arr] of Object.entries(gs.army)) {
    ...
    arr.forEach((c, star) => {
      ...
      dps += (st.atk / Math.max(0.05, st.cd)) * c;
      hp += st.hp * c;
    });
  }
  for (const lvl of gs.turrets) {
    if (lvl > 0) {
      const t = turretStats(lvl, gs.tech.turretAtk, gs.tech.turretSpd);
      dps += t.dps;
      hp += t.hp;
    }
  }
  const bs = baseStats(gs.buildings.base, gs.tech.turretAtk, gs.tech.turretSpd);
  dps += bs.atk / bs.cd;
  hp += bs.hp;
  return { dps, hp };
}

/** 同屏怪物上限：等级越高、大防守越多，用来铺开大规模战斗 */
export function onScreenEnemyCap(baseLevel: number, big: boolean) {
  const n = Math.round(64 + Math.max(1, baseLevel) * 2.4);
  return big ? Math.round(n * 1.4) : n;
}

/**
 * 每秒刷怪预算。个体变弱后，用更多怪来填场面。
 * 不按玩家战力加怪，避免练兵越多越打不过。
 */
export function waveBudgetRate(gs: GameState, big: boolean) {
  const L = Math.max(1, gs.buildings.base);
  const week = Math.floor(gs.attackCount / BIG_EVERY);
  const baseRate = (1.35 + 0.16 * L) * (1 + 0.025 * Math.min(week, 20));
  return baseRate * (big ? 1.45 : 1) * (gs.difficulty ?? 1);
}
