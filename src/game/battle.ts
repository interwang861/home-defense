...
import {
  ...
  monsterUnlockedCount, monsterMinTier, onScreenEnemyCap, waveBudgetRate, tierOf, turretStats, unitStats, BUILDINGS,
} from './config';
import { computeResDelta } from './logic';
import { buildingSprite } from './sprites';
import { TURRET_SFX, sfx, weaponSfx, type SfxKey } from './sound';
...
function pickType(b: BattleState): number {
  const lo = Math.min(b.minTier, b.unlocked - 1);
  let total = 0;
  for (let i = lo; i < b.unlocked; i++) total += (i - lo + 1) * (i - lo + 1);
  let r = Math.random() * total;
  for (let i = lo; i < b.unlocked; i++) {
    r -= (i - lo + 1) * (i - lo + 1);
    if (r <= 0) return i;
  }
  return b.unlocked - 1;
...
export function createBattle(gs: GameState): BattleState {
  ...
  const mul = monsterMultipliers(n, gs.buildings.base, gs.difficulty ?? 1);
  const b: BattleState = {
    ...
    minTier: monsterMinTier(n), rate: waveBudgetRate(gs, big),
    enemyCap: Math.min(MAX_ENEMIES, onScreenEnemyCap(gs.buildings.base, big)),
  };
  ...
  (Object.keys(LAYOUT) as BuildingKey[]).forEach((key) => {
    ...
    if (key === 'base') {
      ...
      bd.gun = {
        atk: bs.atk, cd: bs.cd, cdLeft: 0, range: bs.range, splash: 0, kind: 'shell', shot: '#fde047',
        angle: 0, fired: 0, px: x, py: y - size * 0.62, tier: 0, snd: 'cannon', sndScale: 1.7,
      };
    }
    b.buildings.push(bd);
  });
  gs.turrets.forEach((lvl, i) => {
    ...
    b.buildings.push({
      ...
      gun: {
        atk: st.atk, cd: st.cd, cdLeft: Math.random() * st.cd, range: st.range, splash: st.splash,
        kind: st.tier.kind, shot: st.tier.shot, angle: Math.atan2(y - CY, x - CX), fired: 0, px: x, py: y - 22, tier: t,
        snd: TURRET_SFX[st.tier.kind] ?? 'gun', sndScale: 1 + t * 0.12,
      },
    });
  });
  ...
  for (const { id, star } of units) {
    ...
    releaseTimer[bKey] = (releaseTimer[bKey] ?? 0) + 1;
    const batch = u.air ? 4 : 12;
    const wave = Math.floor((releaseTimer[bKey] - 1) / batch);
    ...
    b.ents.push({
      ...
      shot: u.shot, beam: !!u.beam, star, alt: 0, dir: 1, atkAnim: 0, out: false,
      releaseAt: 0.12 + wave * (u.air ? 0.28 : 0.2), ox: rand(-14, 14), oy: rand(-14, 14),
      snd: weaponSfx({ range: u.range, splash: u.splash, beam: u.beam, cd: u.cd, air: u.air, id: u.id }),
      sndScale: 0.75 + u.size / 60,
    });
  }
...
function rallyPoint(b: BattleState, e: BEnt): [number, number] {
  const ring = Math.floor(Math.sqrt(e.slot / (e.air ? 8 : 12)));
  const start = ring * ring * (e.air ? 8 : 12);
  const count = Math.max(8, (ring + 1) * (ring + 1) * (e.air ? 8 : 12) - start);
  const a = ((e.slot - start) / count) * Math.PI * 2 + ring * 0.17 + (e.air ? b.time * 0.2 : 0);
  const r = Math.min(e.air ? 420 : 520, (e.air ? 130 : 170) + ring * 24);
  return [CX + Math.cos(a) * r, CY + Math.sin(a) * r * (e.air ? 0.72 : 0.86)];
}
...
function spawnMonster(b: BattleState, typeIdx: number, boss = false) {
  ...
  b.ents.push({
    ...
    snd: null, sndScale: 1, // 怪物不发声，避免几百只同时攻击时音效糊成一片
  });
...
export function stepBattle(b: BattleState, dt: number, render = true) {
  ...
  const rate = b.rate * (1 + 0.8 * progress);
  b.budget = Math.min(4000, b.budget + rate * dt);
  let enemyCount = 0;
  for (const e of b.ents) if (e.side === 'enemy') enemyCount++;
  while (enemyCount < b.enemyCap && b.budget >= MONSTERS[b.nextType].cost && b.time < b.duration - 5) {
  ...
  for (const bd of blds) {
    ...
    if (best) {
      ...
      if (g.cdLeft <= 0 && Math.abs(diff) < 0.5) {
        ...
        if (render) sfx(g.snd, g.sndScale);
  ...
  for (const e of b.ents) {
    ...
    e.rx = rx;
    e.ry = ry;
    // 地面单位：主动索敌。但用「牵引绳」限制追击范围，
    // 不让它们一路追到地图边缘，把基地和建筑空出来。
    const LEASH = 560; // 离基地最远追到这里
    const AGGRO = 620; // 自身索敌半径
    ...
    for (const m of enemies) {
      ...
      // 怪物在牵引范围内，或已经贴到脸上（射程内），都要打
      const reachable = d2(m.x, m.y, CX, CY) < LEASH + e.range;
      if (d < AGGRO && (reachable || d <= e.range + m.size * 0.3)) {
        td = d;
        target = m;
      }
    }
    ...
    if (target && canAct) {
      ...
      if (td <= e.range + target.size * 0.25) {
        if (e.cdLeft <= 0) {
          ...
          if (render && e.snd) sfx(e.snd, e.sndScale);
        ...
        let nx = e.x + (dx / L) * e.speed * dt;
        let ny = e.y + (dy / L) * e.speed * dt;
        if (!e.air) {
          // 超出牵引绳就贴着圆周走，不再往外追
          const od = d2(nx, ny, CX, CY);
          const maxR = 560 + e.range * 0.5;
          if (od > maxR) {
            const k = maxR / od;
            nx = CX + (nx - CX) * k;
            ny = CY + (ny - CY) * k;
